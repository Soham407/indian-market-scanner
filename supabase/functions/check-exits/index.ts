...
import { shouldMoveToBreakeven, shouldTimeStop } from "../_shared/trade-math.ts";
...
type OpenTrade = {
  ...
  instruments:
    | { symbol: string; name: string; last_price: number | null }[]
    | { symbol: string; name: string; last_price: number | null }
    | null;
};
...
Deno.serve(async () => {
  ...
  const { data: openTrades, error: tradesError } = await supabase
    ...
    .select(
      "id,instrument_id,side,entry_price,entry_time,stop_loss_price,target_price,shares,risk_amount,stop_moved_to_breakeven,instruments(symbol,name,last_price)",
    )
    .eq("status", "open");
  ...
  for (const trade of openTrades as OpenTrade[]) {
    ...
    // Closes the trade at exitPrice (pre-slippage) and records the outcome.
    const closeTrade = async (
      exitPrice: number,
      exitReason: "stop" | "target" | "time_stop",
      slippageMult: number,
    ) => {
    ...
    if (trade.side === "long" && candle.low <= trade.stop_loss_price) {
      await closeTrade(trade.stop_loss_price, "stop", 0.0010);
      continue;
    }
    if (trade.side === "short" && candle.high >= trade.stop_loss_price) {
      await closeTrade(trade.stop_loss_price, "stop", 0.0010);
      continue;
    }
    ...
    if (trade.side === "long" && candle.high >= trade.target_price) {
      await closeTrade(trade.target_price, "target", 0.0005);
      continue;
    }
    if (trade.side === "short" && candle.low <= trade.target_price) {
      await closeTrade(trade.target_price, "target", 0.0005);
      continue;
    }

    // ── No exit hit: breakeven move, then time-stop ──────────────────────────
    const unrealizedGross = trade.side === "long"
      ? (candle.close - trade.entry_price) * trade.shares
      : (trade.entry_price - candle.close) * trade.shares;
    const ageMinutes = (new Date(candle.candle_open_at).getTime() - new Date(trade.entry_time).getTime()) / 60000;

    if (shouldMoveToBreakeven(unrealizedGross, trade.risk_amount, trade.stop_moved_to_breakeven)) {
      // At +1R the stop moves to entry (one-way): worst case becomes ~breakeven.
      const { error: beError } = await supabase
        .from("bot_paper_trades")
        .update({
          stop_loss_price: trade.entry_price,
          stop_moved_to_breakeven: true,
        })
        .eq("id", trade.id);
      if (!beError) breakevenMoves++;
      continue;
    }

    if (shouldTimeStop(ageMinutes, unrealizedGross, trade.risk_amount)) {
      // Momentum-dead trade: not at +0.5R after 60 minutes — exit at market
      // instead of drifting to EOD paying charges for nothing.
      await closeTrade(candle.close, "time_stop", 0.0005);
    }
  }

  return Response.json({
    status: "Exit check complete",
    exits_processed: exitsProcessed,
    breakeven_moves: breakevenMoves,
  });
});
